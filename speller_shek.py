...
'''Notes:
- Press Q to quit
- Keyboard is replaced with just Y and N 
...
class Keyboard(object):
    ...
    def add_key(self, name, size, pos, images=["black.png", "white.png"]):
        ...
        for i in range(len(images)):
            ...
            image = images[i] 
            # image = visual.ImageStim(win=self.window, image=images[i], units='pix', size=size,autoLog= False)   
            
            # mask = visual.Circle(win=self.window, units='pix', radius=radius, fillColor=[0, 0, 0], autoDraw=True)  
            
            # image.mask = mask
            # image.interpolate = True  
            
            # self.keys[name].append(image)
